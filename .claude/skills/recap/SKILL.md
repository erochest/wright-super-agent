...
"Before we build anything, let's look at what we built last time. The records exist for a reason." — WRIGHT
...
|---------|-------|----------|
| No journal entries found | First session or journals cleaned up | Fall back to git history + MEMORY.md only |
...
- **MEMORY.md** is always loaded into system prompt; `/recap` adds transient session context on top
