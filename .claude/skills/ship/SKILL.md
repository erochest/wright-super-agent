...
"A clean delivery pipeline is the difference between work that lands and work that accumulates. Verify, commit, push — in that order, every time." — WRIGHT
...
Optionally assign issue and update board (if `gh` available and issue provided):
```bash
...
```
Branch:  feat/42-add-powerup-system
Base:    main (synced)
Issue:   #42 — Add power-up system (assigned)
...
Co-Authored-By: WRIGHT <eric.rochester@telusdigital.com>
...
# Confirm pushed (should be empty after ship)
